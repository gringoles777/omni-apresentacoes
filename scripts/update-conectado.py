#!/usr/bin/env python3
"""
Atualiza leads-responderam-conectado.html
Etapa: Conectado (stage 422) — SDR-Novo
Roda via GitHub Actions seg–sáb às 8h, 10h, 12h, 14h, 16h e 18h BRT
"""
...
GCHAT_WEBHOOK = "https://chat.googleapis.com/v1/spaces/AAQADhKrhJ8/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=BYk-2zqa3aEhx5rnYFWE87X9uc5U8ONgmKTCNcKfElw"
STAGE_ID = 422  # Conectado
REPORT_URL = "https://gringoles777.github.io/omni-apresentacoes/leads-responderam-conectado.html"
...
def send_gchat(total_deals, total, layslla_count, david_count):
    msg = {
        "text": (
            f"📊 *Relatório Conectados atualizado* — {LAST_UPDATE}\n"
            f"• {total_deals} leads na etapa · {total} com conversa ativa\n"
            f"• Layslla: {layslla_count} · David: {david_count}\n"
            f"👉 {REPORT_URL}"
        )
    }
    data = json.dumps(msg).encode("utf-8")
    req = urllib.request.Request(GCHAT_WEBHOOK, data=data, headers={"Content-Type": "application/json"})
    try:
        with urllib.request.urlopen(req, timeout=10) as r:
            print(f"Google Chat notificado: {r.status}")
    except Exception as e:
        print(f"  [WARN] Google Chat: {e}", file=sys.stderr)

# ── passo 1: coletar deals do stage 422 ──────────────────────────────────────

print("Coletando deals do Pipedrive stage 422 (Conectado)...")
stage_deals = {}
phone_to_deal = {}
start = 0
while True:
...
print(f"Total stage 422: {total_deals} deals")
...
<title>Leads Conectados · OMNI Brasil</title>
...
<style>
  :root {{
    ...
    --green:#4CAF6F; --green-10:rgba(76,175,111,0.10); --green-20:rgba(76,175,111,0.20);
  }}
  *{{margin:0;padding:0;box-sizing:border-box}}
  ...
  .label{{font-family:'IBM Plex Mono',monospace;font-size:0.6rem;font-weight:500;letter-spacing:0.1em;text-transform:uppercase;color:var(--green)}}
...
<div class="hero">
  <div class="container">
    ...
    <h1>Leads Conectados</h1>
    <p class="hero-subtitle">Etapa Conectado (422) — leads que responderam e estão aguardando agendamento (excluídos: automáticos, "oi" único e sem conteúdo)</p>
...
<div class="stats-section">
  <div class="container">
    <div class="stats-grid">
      ...
      <div class="stat-card"><div class="stat-number">{total}</div><div class="stat-label">Com conversa ativa</div></div>
...
<div class="sdr-section">
  <div class="container">
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>#</th><th>Nome</th><th>Telefone</th><th>Links</th><th>Última mensagem recebida</th></tr></thead>
        <tbody>
{layslla_rows}
        </tbody>
      </table>
...
<div class="footer">
  <div class="container">
    <p class="footer-text">OMNI Brasil · Uso interno · {total} leads com conversa ativa de {total_deals} na etapa Conectado · {TODAY}</p>
  </div>
</div>
...
output_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "leads-responderam-conectado.html")
...
# ── passo 5: notificar Google Chat ────────────────────────────────────────────
send_gchat(total_deals, total, len(layslla), len(david))
